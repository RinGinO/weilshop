...
export default function AdminTaskCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await adminTasksApi.create(formData);
      router.push('/admin/tasks');
    } catch (error: any) {
      alert(error.response?.data?.message || 'Ошибка при создании задачи');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const addFaq = () => {
    const newFaq: CreateCareTaskFaqDto = {
      question: '',
      answer: '',
      order: formData.faq!.length + 1,
    };
  ...
  return (
    <div className="max-w-4xl">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#000000] mb-2">Новая задача ухода</h1>
        <p className="text-[#5B6470]">Создание задачи ухода за автомобилем</p>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="w-full px-4 py-3 rounded-lg border border-[#D9DCDD] focus:outline-none focus:ring-2 focus:ring-[#000000]"
                placeholder="Подробное описание задачи ухода..."
              />
            </div>
          </div>
        </div>
